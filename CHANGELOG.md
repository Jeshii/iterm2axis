...
- Removed `frontIsITerm` precondition from sidebar click handling: sidebar now responds to clicks even when iTerm is not frontmost, with occlusion check (`hs.window.atPoint`) to prevent absorbing clicks when a non-iTerm window is visually on top of the canvas
